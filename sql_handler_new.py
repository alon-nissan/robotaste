...
def init_database() -> bool:
    ...
    try:
        ...
        with open('robotaste_schema.sql', 'r') as f:
            schema_sql = f.read()
    ...
    try:
        with get_database_connection() as conn:
            ...
            return {
                ...
                "questionnaire_data": json.loads(row["questionnaire_data"]) if row["questionnaire_data"] else None,
                "state": row["state"],
                "current_phase": row["current_phase"],
                "experiment_config": json.loads(row["experiment_config"]) if row["experiment_config"] else {},
...
def save_sample_cycle(
    ...
    try:
        ...
        with get_database_connection() as conn:
            ...
            conn.commit()
            logger.info(f"Saved sample {sample_id} for session {session_id}, cycle {cycle_number}")
            return sample_id
    ...
    try:
        with get_database_connection() as conn:
            ...
            return {
                ...
                "selection_data": json.loads(row["selection_data"]) if row["selection_data"] else None,
...
def extract_target_variable(
    ...
    try:
        ...
        try:
            from questionnaire_config import QUESTIONNAIRE_TYPES
        except ImportError:
        ...
        q_def = QUESTIONNAIRE_TYPES.get(questionnaire_type)
        if not q_def:
...
def get_bo_config(session_id: str) -> Dict:
    ...
    try:
        with get_database_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM bo_configuration WHERE session_id = ?", (session_id,))
            row = cursor.fetchone()
    ...
    try:
        ...
        for sample in samples:
            row = {
                ...
                **{f"concentration_{k}": v for k, v in sample["ingredient_concentration"].items()},

                # Unpack selection data
                **{f"selection_{k}": v for k, v in (sample["selection_data"] or {}).items()},
...
def get_participant_target_values(participant_id: str, session_id: str, only_final: bool = True) -> pd.DataFrame:
    """
    DEPRECATED: Use get_training_data() instead.
    Kept for backward compatibility with bayesian_optimizer.py
    """
    logger.warning("get_participant_target_values() is deprecated, use get_training_data()")
