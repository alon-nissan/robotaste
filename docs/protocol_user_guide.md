...
5. Give session code to participant (or scan QR code)

### How Subjects Access the Experiment

Subjects access the experiment through a dedicated experiment page:

**URL Format:**
```
https://your-server.com/experiment?session=ABC123&role=subject
```

The QR code generated by the moderator interface automatically includes this URL. When subjects scan the QR code or enter the session code, they are routed to the experiment page which uses the **PhaseRouter** to navigate through the protocol's phase sequence.

### What Happens During the Session

The protocol controls:
- ✅ Which phases run and in what order (from `phase_sequence`)
- ✅ Custom phases like welcome videos, surveys, breaks
...
**Phase Flow Example (with custom phases):**
1. **Consent** (builtin) - Participant agrees to participate
2. **Welcome Video** (custom) - Training or introduction
3. **Pre-Survey** (custom) - Background questions
4. **Registration** (builtin) - Demographics
5. **Experiment Loop** - Cycles through selection → loading → questionnaire
6. **Post-Survey** (custom) - Final feedback
7. **Completion** (builtin) - Thank you screen

**Cycle Flow (inside experiment_loop):**
- **Cycle 1-2 (Predetermined):** System loads predetermined sample → questionnaire
- **Cycle 3-5 (User Selected):** Participant chooses sample → questionnaire
- **Cycle 6+ (BO Selected):** BO suggests sample → questionnaire
...
3. View:
   ...
   - Current phase (consent, selection, questionnaire, custom phases, etc.)
   - Mode being used
...
## Customizing Phase Flow

> **New in January 2026:** RoboTaste now supports fully customizable phase sequences via the `phase_sequence` protocol field. See [protocol_schema.md](protocol_schema.md#phase-sequence-multipage-architecture) for the complete reference.

### Understanding Phase Sequence

The `phase_sequence` field lets you define exactly which phases run and in what order. This replaces the old fixed state machine flow.

**Default Flow (if phase_sequence omitted):**
```
...
### Adding a Phase Sequence

Add a `phase_sequence` section to your protocol JSON:
...
| `custom` | Protocol-defined phases (text, media, survey, break) |
| `loop` | The experiment cycle (selection → loading → questionnaire → repeat) |

---

## Creating Custom Phases

Custom phases let you add welcome screens, training videos, surveys, and rest breaks without writing code.

### Text Phase - Instructions & Welcome

Display formatted text with optional images:
...
```json
{
  ...
  "content": {
    ...
    "text": "Thank you for participating!\n\n**What to expect:**\n- 20 taste samples\n- ~30 minutes total\n- Rate each sample on a scale",
    "image_url": "https://example.com/welcome.png"
  }
}
```

**Use for:** Welcome messages, instructions, experiment explanations

### Media Phase - Videos & Images

Show training videos or visual stimuli:
...
**Use for:** Training videos, visual instructions, stimuli presentation

**Supported formats:**
- Video: MP4, WebM (hosted URL)
- Image: PNG, JPG, GIF (hosted URL)

### Survey Phase - Custom Questions

Collect additional data from participants:
...
```json
{
  ...
  "content": {
    "type": "survey",
    "title": "Pre-Experiment Questions",
    "questions": [
      {
        "id": "age",
        "type": "text",
      ...
      },
      {
        "id": "sweet_preference",
        "type": "radio",
        "label": "How would you describe your sweet preference?",
        "options": ["Love sweets", "Neutral", "Prefer savory"],
        "required": true
      },
      {
        "id": "dietary",
      ...
      },
      {
        "id": "hunger",
        "type": "slider",
        "label": "How hungry are you right now?",
        "min": 0,
        "max": 100,
        "default": 50
      }
...
### Break Phase - Rest Periods

Add timed breaks for palate cleansing:
...
```json
{
  ...
  "content": {
    "type": "break",
    "title": "Rest Break",
    "message": "Take a moment to rest. Rinse your mouth with water.",
    "duration": 60,
    "show_countdown": true
  }
}
...
**Use for:** Rest periods, palate cleansing, fatigue reduction

---

## Example: Complete Protocol with Custom Phases

```json
{
  "name": "Full Study Protocol",
  "version": "1.0",
  "phase_sequence": {
    "phases": [
      {"phase_id": "consent", "phase_type": "builtin", "required": true},
      {
        "phase_id": "intro_video",
        "phase_type": "custom",
        "content": {
          "type": "media",
          "title": "Welcome",
      ...
      {"phase_id": "experiment_loop", "phase_type": "loop", "required": true},
      {
        ...
        "content": {
          ...
          "questions": [
            {"id": "experience", "type": "slider", "label": "Rate your experience", "min": 0, "max": 100}
          ]
        }
      },
      {"phase_id": "completion", "phase_type": "builtin", "required": true}
...
#### Custom phase not rendering

**Problem:** Custom phase shows error or doesn't display

**Solution:**
- Verify `phase_type` is set to `"custom"`
- Check that `content.type` is one of: `text`, `media`, `survey`, `break`
- Ensure all required fields for the content type are present
- Check browser console for JavaScript errors if using external media URLs

#### Phase not advancing

**Problem:** Subject clicks "Continue" but phase doesn't change

**Solution:**
- This is usually a phase renderer issue
- For custom phases, verify the content structure is valid
- Try refreshing the page to resync with database
- Check moderator view for the actual current phase

#### "Unknown phase" error

**Problem:** PhaseRouter can't find the requested phase

**Solution:**
- Verify the `phase_id` in your `phase_sequence` matches a valid builtin phase or has proper custom content
- Valid builtin phases: `consent`, `registration`, `loading`, `selection`, `questionnaire`, `robot_preparing`, `completion`
- For custom phases, ensure `content` object is present
